...
function compare(){
  ...
  const table=(heads,rows)=>`<div class="compare-table"><div class="compare-row head">${heads.map(h=>`<div>${h}</div>`).join('')}</div>${rows.map(r=>`<div class="compare-row">${r.map(c=>`<div>${c}</div>`).join('')}</div>`).join('')}</div>`;
  return `<main class="screen"><div class="topbar"><button onclick="setState({view:'home'})">🏠 홈</button><span>› 한눈에 비교</span></div><section class="page-title"><h1>📊 한눈에 비교</h1><p>탐험으로 배우고, 비교로 정리합니다.</p></section><section class="compare-card"><h2>🎉 절기 비교</h2>${table(['절기','핵심','예수님'],festivalRows)}</section><section class="compare-card"><h2>🔥 제사 비교</h2>${table(['제사','목적','핵심'],sacrificeRows)}</section><section class="compare-card summary-card"><h2>💎 핵심 한눈에</h2><p><b>🎉 절기</b><br>시간을 통해 하나님의 구원 계획을 보여 줍니다.</p><p><b>🔥 제사</b><br>방법을 통해 하나님께 나아가는 길을 보여 줍니다.</p><button class="home-return" onclick="setState({view:'home'})">🏠 홈으로</button></section></main>`;
}
...
function render(){app.innerHTML = state.view==='home'?home():state.view==='list'?list():state.view==='compare'?compare():card(); app.insertAdjacentHTML('beforeend', modal());}
